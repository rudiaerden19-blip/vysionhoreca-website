...
/** Fiscale rapportdatum voor een order (00:00 – +1dag 12:00). */
export function fiscalReportDateForOrderCreatedAt(createdAt: string): string | null {
  ...
  const candidates = [center, addDaysToBelgiumYMD(center, -1), addDaysToBelgiumYMD(center, 1)]
  for (const ymd of candidates) {
    const { startUTC, endUTC } = getZRapportDateBounds(ymd)
    if (t >= new Date(startUTC) && t <= new Date(endUTC)) return ymd
  }
  return center
}
