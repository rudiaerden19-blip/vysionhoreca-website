/**
 * IndexedDB voor GKS-kassa — aparte database naam (niet vysion-kassa-offline).
 */

const DB_NAME = 'vysion-gks-kassa-offline'
const DB_VERSION = 1
const STORE = 'kv'
...
export function menuCacheKey(tenant: string) {
  return `gks_menu_v1_${tenant}`
}

export function offlineQueueDbKey(tenant: string) {
  return `gks_offline_orders_v1_${tenant}`
}
...
export async function offlineDbGetOrderQueue(tenant: string): Promise<object[]> {
  const raw = await offlineDbGet<object[]>(offlineQueueDbKey(tenant))
