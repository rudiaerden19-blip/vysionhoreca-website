...
import { getMenuCategories, getMenuProducts } from '@/lib/admin-api'
import { buildCategoryVatLookup } from '@/lib/order-vat'
...
export async function fetchZReportVatContextForTenant(tenantSlug: string): Promise<ZReportVatContext> {
  ...
  return buildZReportVatContext(categories, products)
}
