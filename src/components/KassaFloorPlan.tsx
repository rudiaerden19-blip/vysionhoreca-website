...
import { kassaFloorPlanViewportLayerClass } from '@/lib/kassa-floor-plan-surface'
import {
...
export default function KassaFloorPlan({
  ...
  const floorChromeStyle = useMemo(
    () => ({
      cursor: isLocked ? (isPanningFloor ? 'grabbing' : 'grab') : 'default',
      touchAction: 'none' as const,
    }),
    [isLocked, isPanningFloor],
  )
  ...
  const viewportLayerRef = useRef<HTMLDivElement | null>(null)
  ...
  const resolveFloorHit = useCallback((clientX: number, clientY: number): { kind: 'table' |  'decor'; id: string } | null => {
    const root = viewportLayerRef.current
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col ${KASSA_POS_MENU_PLATE_SHELL_BG_CLASS}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Floor plan — pan/zoom op korrel/raster (binnen + terras) */}
        <div
          ref={canvasRef}
          className="floor-plan relative flex-1 select-none overflow-hidden bg-[#1a1a1a]"
        ...
        >
          <div
            ...
            className={`absolute inset-0 origin-top-left ${kassaFloorPlanViewportLayerClass(planZone)}`}
            style={{
          ...
          >
            <button
              type="button"
              aria-label={t('reservationKassa.zoomInAria')}
              onClick={e => {
                e.stopPropagation()
                stepFloorZoom(1.22)
              }}
            ...
            <button
              type="button"
              aria-label={t('reservationKassa.zoomOutAria')}
              onClick={e => {
                e.stopPropagation()
