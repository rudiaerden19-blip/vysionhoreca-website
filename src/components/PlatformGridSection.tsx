...
export default function PlatformGridSection({
  ...
  itemOverrides?: Partial<Record<string, PlatformGridItemOverride>>
} = {}) {
  const { t } = useLanguage()
  ...
  return (
    <section id="platform" className="py-20 sm:py-28 lg:py-36 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-5 sm:gap-6 lg:gap-7 items-stretch auto-rows-fr">
          {PLATFORM_PAGES.map(({ slug, msgKey, cardHeaderImage }) => {
            const override = itemOverrides?.[slug]
            const title = t(override?.titleKey ?? `platform.${msgKey}.title`)
            const teaser = t(override?.teaserKey ?? `platform.${msgKey}.teaser`)
            const headerImage = override?.cardHeaderImage ?? cardHeaderImage
            const headerAlt = override?.cardHeaderAltKey
              ? t(override.cardHeaderAltKey)
              : cardHeaderImage && t(`platform.${msgKey}.cardHeaderAlt`) !== `platform.${msgKey}.cardHeaderAlt`
                ? t(`platform.${msgKey}.cardHeaderAlt`)
                : ''
            const imageClass = override?.headerImageContain
              ? 'object-contain object-center p-3 transition-transform duration-300 group-hover:scale-105'
            ...
            return (
              <div key={slug} className={cardShellClasses}>
                <div
                  className={`relative h-28 sm:h-32 w-full shrink-0 overflow-hidden rounded-t-2xl ${
                    override?.headerImageContain
                      ? 'bg-white'
                ...
                <div className="flex flex-1 flex-col items-center justify-start px-4 pb-5 pt-4 sm:px-6 sm:pb-6 sm:pt-5">
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2.5 leading-snug px-1 min-h-[2.75rem] sm:min-h-[3rem] flex items-center justify-center">
                    {title}
                  </h3>
                  <p className="flex-1 w-full text-xs sm:text-sm md:text-base text-gray-600 leading-relaxed break-words [overflow-wrap:anywhere]">
                    {teaser}
                  </p>
                </div>
              </div>
