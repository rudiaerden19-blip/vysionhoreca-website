...
import {
  ...
  KASSA_SPEELS_MENU_TILE_IMAGE_WELL,
  KASSA_SPEELS_MENU_TILE_IMAGE_WELL_SXGA,
  KASSA_SPEELS_MENU_TILE_IMG_FRAME,
  KASSA_SPEELS_MENU_TILE_IMAGE_FILM,
...
/** 15″ breedbeeld — smaller dan 17″ SXGA, anders oogt de zijbalk ingezoomd. */
const KASSA_SIDEBAR_WIDTH_WIDE15_PX = 268
const KASSA_SIDEBAR_FOOTER_LEFT_COL_WIDE15 =
  'w-[5.25rem] min-w-[5.25rem] max-w-[5.25rem] shrink-0'

type KassaChromeDensity = 'wide15' | 'sxga' | 'default'
...
function kassaFloorZoneButtonTouchClass(density: KassaChromeDensity): string {
  if (density === 'wide15') return 'min-h-[2.1rem] py-1.5'
  if (density === 'sxga') return 'min-h-[3.25rem] py-2.5'
  return 'min-h-[2.75rem] py-2'
}

function kassaOrderTypeButtonTouchClass(density: KassaChromeDensity): string {
  if (density === 'wide15') return 'min-h-[2.1rem] py-1.5'
  if (density === 'sxga') return 'min-h-[3.25rem] py-2.5'
  return 'min-h-[3rem] py-2.5'
}

/** Sidebar-footer: touch-vriendelijke hoogte (BTW-bon / Bon / Verwijder). */
function kassaFooterActionTouchMinHClass(density: KassaChromeDensity, denseBill: boolean): string {
  if (density === 'wide15') return 'min-h-[2.55rem] py-1.5'
  if (density === 'sxga') return 'min-h-[4.75rem] py-2.5'
...
type KassaTileLook = 'light' | 'dark' | 'luxury' | 'speels'

function kassaTileLook(posLuxury: boolean, classicDark: boolean, speels: boolean): KassaTileLook {
  if (speels) return 'speels'
...
const KassaCategoryTileButton = memo(function KassaCategoryTileButton({
  ...
  speelsAppearance = false,
}: KassaCategoryTileButtonProps) {
  const look = kassaTileLook(posLuxuryAppearance, classicDarkAppearance, speelsAppearance)
  if (look === 'dark') {
    return (
      <button
        type="button"
  ...
  const luxuryChrome = look === 'luxury' || look === 'speels'
  const btnClass =
    luxuryChrome
  ...
  const imgFrame = look === 'speels'
    ? KASSA_SPEELS_MENU_TILE_IMG_FRAME
    : posLuxuryAppearance
      ? KASSA_POS_MENU_TILE_IMG_FRAME
      : 'relative h-full w-full min-h-0'
  const imgFilm = look === 'speels'
    ? KASSA_SPEELS_MENU_TILE_IMAGE_FILM
    : posLuxuryAppearance
      ? KASSA_POS_MENU_TILE_IMAGE_FILM
      : null
      ...
      {imageUrl ? (
        <>
          <div className={imgWell}>
            ...
            {imgFilm ? <div className={imgFilm} aria-hidden /> : null}
          </div>
...
const KassaProductTileButton = memo(function KassaProductTileButton({
  ...
  if (look === 'dark') {
    return (
      ...
      >
        ...
        {hasOpts ? (
          <div className="absolute top-1.5 left-1.5 z-20 rounded-md bg-white/20 px-1.5 py-0.5 text-[10px] font-bold text-white">
            {t('kassaApp.optionBadgeShort')}
          </div>
        ) : null}
      </button>
    )
  ...
  const noImgTop =
    look === 'speels'
      ? sxgaDenseTileLayout
        ? `${KASSA_SPEELS_MENU_TILE_PLACEHOLDER_WELL_SXGA} pt-2`
        : `${KASSA_SPEELS_MENU_TILE_PLACEHOLDER_WELL} pt-4`
      : look === 'luxury'
      ? sxgaDenseTileLayout
...
function KassaAdminPageInner({ params }: { params: { tenant: string } }) {
  ...
  const kassaDarkHeaderBtnShell = kassaWide15Chrome
  ...
  const headerQuickLinkBtnClass = kassaWide15Chrome
    ...
    : kassaLight
      ...
      : kassaClassicDark
        ? KASSA_CLASSIC_HEADER_QUICK_LINK_BTN
  ...
  const kassaChromeDensity: KassaChromeDensity = kassaWide15Chrome
    ? 'wide15'
    : kassaSxgaDenseTiles
      ? 'sxga'
      : 'default'
  const kassaSidebarFooterLeftCol = kassaWide15Chrome
    ? KASSA_SIDEBAR_FOOTER_LEFT_COL_WIDE15
    : KASSA_SIDEBAR_FOOTER_LEFT_COL
      ...
      <div key={item.cartKey} className={ui.cartRowBg}>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            ...
            className={
              kassaAppearanceDark
                ? `touch-manipulation ${kassaPosCartQtyButtonClass(cartLineQtyBtnCompact, posChrome)}`
          ...
          {!demoViewOnly &&
            ...
            productIdsWithOptionsSet.has(item.product.id) && (
              <button
                ...
                className={
                  kassaAppearanceDark
                    ? `touch-manipulation ${kassaPosCartQtyButtonClass(cartLineQtyBtnCompact, posChrome)} text-sm`
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-30 flex w-full min-w-0 shrink-0 items-center ${
          kassaWide15Chrome ? 'min-h-[44px] gap-1 px-1.5 py-1' : 'min-h-[56px] gap-1.5 px-2 py-2 sm:gap-2 sm:px-3'
        } ${
          kassaPosLuxury
            ? `${kassaWide15Chrome ? 'pb-1.5' : 'pb-3'} ${kassaPlateBgClass}`
            : 'bg-black'
        }`}
      >
        ...
        <div className="relative z-20 flex shrink-0 items-center gap-2">
          <button onClick={() => { setHamburgerOpen(!hamburgerOpen); setHamburgerSubOpen(null) }}
            className={`flex items-center transition-colors ${
              kassaWide15Chrome ? 'gap-1 px-1.5 py-1' : 'gap-1.5 px-2 py-1.5 sm:gap-2 sm:px-3'
            } ${
              kassaPosLuxury
                ? kassaPosButtonClass(true, posChrome)
                : kassaLight
                  ? `rounded-xl ${hamburgerOpen ? KASSA_LIGHT_BTN_FACE_ON : KASSA_LIGHT_BTN_FACE}`
                : `rounded-xl ${hamburgerOpen ? KASSA_CLASSIC_BTN_FACE_ON : KASSA_CLASSIC_BTN_FACE}`
            }`}
            type="button"
                ...
                <div className={`${ui.flyMenuPanel} overflow-y-auto`} style={{ width: 240, maxHeight: '85vh'}}>
                  <div
                    className={`sticky top-0 rounded-t-2xl px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-white ${
                      kassaPosLuxury ? kassaPlateBgClass : 'bg-[#1e293b]'
                    }`}
                  >
                    {t('adminLayout.menu')}
                  </div>
        ...
        <div
          className={`relative z-20 flex min-h-0 min-w-0 flex-1 items-center ${
            kassaWide15Chrome ? 'ml-1.5' : kassaCompactChrome ? 'ml-[2cm]': 'ml-[4cm]'
          }`}
        >
          <nav
          ...
          {KASSA_UI_APPEARANCE_TOGGLE_ENABLED ? (
            <KassaModePicker
              ...
              panelClassName={`overflow-hidden rounded-xl border shadow-xl ${ui.flyMenuBorder} ${kassaPosLuxury ? kassaPlateBgClass : kassaAppearanceDark ? 'bg-[#151a21]' : 'bg-white'}`}
            ...
            <div
              className={`inline-flex max-w-[6.5rem] shrink-0 items-center justify-center gap-0.5 leading-tight min-h-[2.35rem] sm:min-h-[2.6rem] sm:max-w-[8rem] md:max-w-none ${KASSA_HEADER_QUICK_LINK_LABEL} ${
                kassaPosLuxury
                  ? isOnline
                    ? headerQuickLinkBtnClass
                    : `${kassaDarkHeaderBtnShell} ${kassaPosButtonClass(false, posChrome)} bg-red-600/95 text-white`
                  : isOnline
                    ? `rounded-xl px-3 py-2 font-bold ${kassaLight ? KASSA_LIGHT_BTN_FACE : KASSA_CLASSIC_BTN_FACE}`
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden w-full">
        ...
        >
          ...
          >
            {menuLoading ? (
              <div data-testid="kassa-menu-loading" className={`flex items-center justify-center min-h-[12rem] text-lg ${ui.menuEmptyMuted}`}>{t('kassaApp.loading')}</div>
            ) : !selectedCategory ? (
              /* Categorieën: responsief raster; rijhoogte vult viewport; gap-4 = KASSA_MENU_GRID_GAP_PX */
              categories.length === 0 ? (
                <div className={`flex flex-col items-center justify-center min-h-[12rem] ${ui.menuEmptyMuted}`}>
                  ...
                  {categories.map((cat, catIndex) => {
                    ...
                    return (
                      <KassaCategoryTileButton
                        ...
                        speelsAppearance={kassaLayout === 'speels'}
                      />
                    )
                  })}
                </div>
              ...
              productsInSelectedCategory.length === 0 ? (
                  ...
                  >
                    {productsInSelectedCategory.map((product, productIndex) => {
              ...
              <button
                ...
                className={`flex min-w-0 flex-1 items-center justify-center px-2 transition-colors sm:px-3 ${kassaFloorZoneButtonTouchClass(
                  ...
                  kassaPosLuxury
                    ...
                    : `rounded-xl font-bold ${kassaZoneTab === 'sales' ? KASSA_CLASSIC_ACTION_BTN_FACE_ON : KASSA_CLASSIC_ACTION_BTN_FACE}`
                }`}
              ...
              <button
                ...
                className={`flex min-w-0 flex-1 flex-col items-center justify-center px-2 transition-colors sm:px-3 ${kassaFloorZoneButtonTouchClass(
                  ...
                  kassaPosLuxury
                    ...
                    : `rounded-xl font-bold ${kassaZoneTab === 'inside' ? KASSA_CLASSIC_ACTION_BTN_FACE_ON : KASSA_CLASSIC_ACTION_BTN_FACE}`
                }`}
              >
        ...
        >
          <div className={`flex ${kassaSidebarRowGapClass}`}>
            <button
              ...
              className={
                kassaPosLuxury
                  ? `flex min-w-0 flex-1 flex-col items-center justify-center text-center px-2 ${kassaOrderTypeButtonTouchClass(kassaChromeDensity)} ${kassaPosButtonClass(orderType === 'DINE_IN', posChrome)}`
            ...
            <button
              ...
              className={
                kassaPosLuxury
                  ? `flex min-w-0 flex-1 flex-col items-center justify-center text-center px-2 ${kassaOrderTypeButtonTouchClass(kassaChromeDensity)} ${kassaPosButtonClass(orderType === 'TAKEAWAY', posChrome)}`
            ...
            <button
              ...
              className={
                kassaPosLuxury
                  ? `flex min-w-0 flex-1 flex-col items-center justify-center text-center px-2 ${kassaOrderTypeButtonTouchClass(kassaChromeDensity)} ${kassaPosButtonClass(orderType === 'DELIVERY', posChrome)}`
              ...
              {numpadValue && parseFloat(numpadValue) > 0 && (
                <button
                  ...
                  className={
                    kassaAppearanceDark
                      ? kassaClassicDark
                        ? `mt-3 shrink-0 touch-manipulation rounded-xl py-4 text-base font-bold ${KASSA_CLASSIC_ACTION_BTN_FACE_ON}`
        ...
        {kassaPosLuxury ? (
          <div
            className={`sticky bottom-0 z-10 shrink-0 ${kassaLayout === 'luxe' ? '' : `border-t ${KASSA_POS_RULE_BLACK}`} ${kassaPlateBgClass} ${
              kassaWide15Chrome ? 'px-2 py-1.5 space-y-1.5' : kassaCompactChrome ? 'px-2.5 py-2 space-y-2': 'px-3 py-2.5 space-y-2.5'
            }`}
          >
            <div
            ...
            >
              <button
                ...
                className={`flex items-center justify-center px-3 ${kassaSidebarFooterLeftCol} ${kassaPosButtonClass(kassaQuickMenuVisible, posChrome)}`}
              ...
              <div
                role="status"
                aria-live="polite"
                className={`flex min-w-0 flex-1 items-center justify-between gap-2 px-2.5 ${kassaPosRaisedStripClass(posChrome)}`}
              >
            ...
            <div className={`flex touch-manipulation select-none ${kassaCompactChrome ? 'gap-2': 'gap-2.5'}`}>
              <button
                ...
                className={`flex items-center justify-center px-3 ${kassaSidebarFooterLeftCol} ${kassaPosButtonClass(numpadPanelVisible, posChrome)} ${
                  kassaWide15Chrome ? 'min-h-[2.75rem] py-2' : kassaCompactChrome ? 'min-h-[4rem] py-3.5': 'min-h-[3.5rem] py-3'
                }`}
              >
          ...
          <div className={`shrink-0 border-t ${
            kassaWide15Chrome ? 'px-2 py-1.5 space-y-1.5' : 'px-3 py-2.5 space-y-2.5'
          } ${
            kassaClassicDark ? 'border-zinc-700' : 'border-gray-200'
          }`}>
            <div
            ...
            >
              <button
                ...
                aria-pressed={kassaQuickMenuVisible}
                onClick={toggleKassaQuickMenu}
                className={`flex items-center justify-center rounded-xl px-3 py-2 text-xs font-bold leading-tight ${kassaSidebarFooterLeftCol} ${
                  kassaLight
                    ? kassaQuickMenuVisible
                      ? KASSA_LIGHT_BTN_FACE_ON
                      : KASSA_LIGHT_BTN_FACE
              ...
              <div
                role="status"
                aria-live="polite"
                className={`flex min-w-0 flex-1 items-center justify-between gap-2 rounded-xl px-2.5 py-2 ${
                  kassaClassicDark
                    ? 'border border-zinc-600 bg-[#151a21]'
                    : 'border border-gray-200 bg-gray-50'
                }`}
              >
                <span className={`text-sm font-bold ${ui.numpadMeta}`}>{t('kassaApp.cartTotal')}</span>
                <span className={`truncate text-right text-xl font-bold tabular-nums ${
                  kassaClassicDark ? 'text-red-500' : 'text-black'
                }`}>
                  €{total.toFixed(2)}
                </span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 touch-manipulation select-none">
              ...
              <button
                ...
                disabled={draftBonLineItems.length === 0 || draftBonPrinting}
                className={`flex flex-col items-center justify-center gap-1 rounded-xl active:brightness-95 disabled:pointer-events-none ${
                  kassaLight
            ...
            <div className="flex gap-2.5 touch-manipulation select-none">
              <button
                ...
                className={`flex items-center justify-center rounded-xl ${kassaWide15Chrome ? 'min-h-[2.6rem] py-1.5 text-xs' : 'min-h-[3.5rem] py-3 text-sm sm:text-base'} font-bold ${kassaSidebarFooterLeftCol} ${
                  kassaLight
                    ? numpadPanelVisible
                      ? KASSA_LIGHT_BTN_FACE_ON
                      : KASSA_LIGHT_BTN_FACE
        ...
        >
          ...
          <div
            className={`relative z-10 flex w-full max-w-3xl max-h-[min(88vh,780px)] min-h-0 flex-col overflow-hidden rounded-2xl border shadow-2xl ${ui.tablePickerBorder} ${
              kassaPosLuxury ? kassaPlateBgClass : kassaAppearanceDark ? 'bg-[#151a21]' : 'bg-white'
            }`}
          >
              ...
              {kassaFloorPlanEnabled && (
                <button
                  ...
                  className={`flex items-center justify-center py-3 text-sm font-semibold ${tableNumber ? 'flex-1': 'w-full'} ${
                    kassaAppearanceDark
                      ? kassaClassicDark
                        ? `rounded-xl ${KASSA_CLASSIC_BTN_FACE}`
                        : kassaPosButtonClass(false, posChrome)
