...
/**
 * GKS 2.0 fork — aparte POS-route voor FDM-integratie (certificatietraject).
 * Productie-kassa blijft ongewijzigd onder /admin/kassa.
 * URL: /shop/[tenant]/gks (pilot buiten /admin)
 */
...
import {
  ...
  gksCustomerDisplayWindowName,
  gksFloorPlanTablesLocalStorageKey,
  gksMenuCatsCacheKey,
  gksMenuOptsCacheKey,
  gksMenuProdsCacheKey,
  gksPaidReceiptDedupeStorageKey,
  gksSettingsCacheKey,
  gksStoolStatusKey,
  gksTableOrdersStorageKey,
} from '@/lib/gks-kassa/storage-keys'
...
const KASSA_HEADER_QUICK_LINK_LABEL =
  'text-sm font-medium leading-snug tracking-[0.04em] text-[#f0f0f0] sm:text-[15px]'

/** Zijbalk-knoppen — iets grotere labels (kiosk). */
const GKS_SIDEBAR_BTN_LABEL =
...
const KASSA_MENU_TILE_LABEL_WRAP =
  `pointer-events-none shrink-0 w-full ${GKS_MENU_TILE_LABEL_SURFACE} px-2.5 pb-2.5 pt-2 sm:px-3 sm:pb-3`
...
const KASSA_MENU_TILE_LABEL_CLASS =
  `m-0 line-clamp-2 text-center text-[17px] !font-medium leading-snug tracking-[0.02em] text-[#e8e8e8] sm:text-lg md:text-xl`

/** SXGA 17″: categorie + product gelijk (`md:text-xl` wordt te groot in smalle rastercel). */
const KASSA_MENU_TILE_LABEL_CLASS_SXGA =
  `m-0 line-clamp-2 text-center text-[16px] !font-medium leading-snug tracking-tight text-white sm:text-[17px]`
...
const KassaProductTileButton = memo(function KassaProductTileButton({
  ...
  return (
    ...
    >
      ...
      {inCart > 0 && (
        <div className="absolute top-1.5 right-1.5 z-20 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500 text-xs font-semibold text-white shadow-md">
          {inCart}
        </div>
      )}
      {hasOpts && (
        <div className="absolute top-1.5 left-1.5 z-20 rounded-md bg-amber-400 px-1.5 py-0.5 text-xs font-semibold text-white shadow">
          ⚙️
        </div>
      )}
    </button>
...
function kassaPaidReceiptDedupeStorageKey(tenantSlug: string, order: KassaLastOrderReceipt): string {
  return gksPaidReceiptDedupeStorageKey(tenantSlug, kassaPaidReceiptGuardKey(order))
}
...
function KassaAdminPageInner({ params }: { params: { tenant: string } }) {
  ...
  const applyKassaFloorPlanTablesPayload = useCallback((zone: FloorPlanZone, raw: unknown): boolean => {
    ...
    setKassaTablesByZone((prev) => {
      ...
      localStorage.setItem(gksFloorPlanTablesLocalStorageKey(tenant, zone), JSON.stringify(fixed))
  ...
  useEffect(() => {
    const cacheKeySettings = gksSettingsCacheKey(tenant)
  ...
  const tableOrdersKey = gksTableOrdersStorageKey(tenant)
  ...
  useEffect(() => {
    if (kassaFloorPlanEnabled) {
      for (const zone of KASSA_FLOOR_ZONES) {
        const lsRaw = localStorage.getItem(gksFloorPlanTablesLocalStorageKey(tenant, zone))
        if (lsRaw) {
          try {
  ...
  // ── Realtime sync: open manden (gks_commercial_orders, niet productie orders) ──
  useEffect(() => {
    const ordersChannel = supabase
      .channel(`gks_kassa_open_orders_${tenant}`)
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'gks_commercial_orders', filter: `tenant_slug=eq.${tenant}` },
        () => {
          void fetchOpenTableOrdersForTenant(tenant).then(applyOpenOrdersFromServerRows)
        }
      )
  ...
  const updateTableStatus = (tblNr: string, occupied: boolean, zone: FloorPlanZone) => {
    const lsKey = gksFloorPlanTablesLocalStorageKey(tenant, zone)
  ...
  // ── Menu cache keys ────────────────────────────────────────────────────────
  const CACHE_CATS = gksMenuCatsCacheKey(tenant)
  ...
  useEffect(() => {
    try {
      const tok = sessionStorage.getItem(gksCustomerDisplaySessionKey(tenant))?.trim()
  ...
  const openKlantschermWindow = useCallback(() => {
    ...
    if (!tok) {
      try {
        const saved = sessionStorage.getItem(gksCustomerDisplaySessionKey(tenant))?.trim()
        if (saved) {
          tok = saved
    ...
    if (!tok) {
      ...
      try {
        sessionStorage.setItem(gksCustomerDisplaySessionKey(tenant), tok)
      } catch {
        /* ignore */
      }
    ...
    const winName = gksCustomerDisplayWindowName(tenant)
  ...
  useGksKassaOfflineFlushBridge(tenant, flushOfflineOrdersRef)
  ...
  const clearTableAfterPayment = (zone: FloorPlanZone, tblNr: string) => {
    ...
    const stoolStatusKey = gksStoolStatusKey(
      tenant,
      zone === FLOOR_PLAN_ZONE_TERRACE,
    )
    try {
  ...
  const completePayment = async (
    ...
    // TODO(GKS): Finale verkoop — event N (signSale) vóór insert orders + Z-sync.
    // 1) FDM status OK (query status); geen verkoop als FDM_NOT_OPERATIONAL / BII TIMEOUT.
    // 2) Map billLines → SaleInput (vat labels A/B/C/D/X, departments, posFiscalTicketNo, bookingPeriodId).
    // 3) financials: paymentLine(s), SPLIT, verplichte ROUNDING bij cash (UC201), geen offline queue zonder signSale.
    // 4) await fdmClient.signSale → vatCalc, fdmRef, verificationUrl, shortSignature, footer.
    // 5) Bewaar GraphQL request/response; setLastOrder met fiscale snapshot.
  ...
  const printReceipt = async (
    ...
    // Draft / toog / keuken (opts.draft): vereist vooraf signOrder of signPreBill (P);
    //   teksten PRO FORMA, VOORLOPIGE REKENING, DIT IS GEEN GELDIG BTW-KASTICKET; keuken zonder bedragen.
    // Betaalde bon: alleen na signSale (N); BTW uit order.fdmVatCalc (FDM vatCalc), niet computeInclusiveVatSplitFromCart;
    //   kop BTW-KASTICKET, fdmRef/eventCounter/shortSignature/footer, QR uit verificationUrl (geen eigen encoder).
    // COPY (C): signCopy + zelfde QR als origineel N.
    // sendToVysionPrintAgent: bonInhoud bouwen uit fiscale snapshot, niet alleen bonLines hieronder.
    if (!order) {
  ...
  const performLogout = () => {
    if (demoViewOnly) {
      clearPublicDemoSession()
      window.location.replace(`/shop/${tenant}/gks?demo=bekijk`)
      return
    }
  ...
  const loadStaffClockList = useCallback(async (opts?: { background?: boolean }) => {
    ...
    try {
      const url = GKS_MANDATORY_STAFF_SESSION
        ...
        : `/api/kassa/staff-clock?tenant_slug=${encodeURIComponent(tenant)}`
  ...
  useEffect(() => {
    ...
    if (!GKS_MANDATORY_STAFF_SESSION && !tenantInfo?.kassa_staff_clock_enabled) {
      setStaffClockListHydrated(true)
      return
    }
  ...
  return (
    <div
      ...
      data-gks-ui="20250608-floor-header-no-zone-btns"
    ...
    >
      ...
      >
        ...
        <div className="relative z-20 flex shrink-0 items-center gap-2">
          ...
          >
            ...
            <span className="font-semibold text-xs leading-tight sm:text-sm">{t('kassaApp.hamburgerMenu')}</span>
          </button>
          {hamburgerOpen && (() => {
        ...
        <div className="relative z-20 ml-[4cm] flex min-h-0 min-w-0 flex-1 items-center">
          ...
          {effectiveAccess.reservaties && (
            ...
            >
              ...
              {pendingReservCount > 0 && (
                <span className="absolute -right-1 -top-1.5 flex h-7 min-w-[26px] items-center justify-center rounded-full border-2 border-white bg-red-600 px-1.5 text-sm font-semibold text-white shadow-lg sm:-right-2 sm:-top-2">
                  {pendingReservCount}
                </span>
              )}
            </button>
          ...
          <button
            ...
            tabIndex={KASSA_HEADER_HIDE_CUSTOMER_DISPLAY_AND_SOUND ? -1 : 0}
            className={`${KASSA_HEADER_QUICK_LINK_BTN} ${
              soundsOn ? '' : `${GKS_BTN_SHAPE} ${GKS_POS_BTN} opacity-90`
            } ${KASSA_HEADER_HIDE_CUSTOMER_DISPLAY_AND_SOUND ? 'hidden' : ''}`}
          ...
          {isOnline !== null && (
            <div
              className={`inline-flex max-w-[6.5rem] shrink-0 items-center justify-center gap-0.5 px-3 py-2 font-semibold leading-tight min-h-[2.35rem] sm:min-h-[2.6rem] sm:max-w-[8rem] md:max-w-none ${KASSA_HEADER_QUICK_LINK_LABEL} ${
      ...
      {isOnline === false && (
        <div className="flex-shrink-0 bg-red-700 text-white text-xs font-semibold flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-1.5 px-4 text-center">
          <span className="font-semibold">{t('kassaApp.offlineModeActive')}</span>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden w-full">
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 touch-pan-x whitespace-nowrap border font-semibold leading-tight transition-colors active:brightness-95 ${
                        kassaSxgaDenseTiles
                          ? 'px-3 py-1.5 text-sm'
        ...
        <div className="shrink-0 px-3 pt-3 pb-3">
            <div className="flex gap-4">
              <button
                ...
                className={`flex flex-1 flex-col items-center justify-center px-3 font-semibold transition-colors ${kassaFloorZoneButtonTouchClass(
                  kassaSxgaDenseTiles,
                )} ${gksPosButtonClass(
                  pickerBrowseZone === FLOOR_PLAN_ZONE_INSIDE && showTablePicker,
                )}`}
              >
                <span className="font-medium tracking-[0.03em]">{t('kassaApp.floorZoneInside')}</span>
              ...
              >
                <span className="font-medium tracking-[0.03em]">{t('kassaApp.floorZoneTerrace')}</span>
        ...
        <div className="mx-3 mb-3 mt-2.5 shrink-0 space-y-3" data-testid="kassa-order-type-bar">
          <div className="flex gap-4">
            <button
              type="button"
          ...
          {orderType === 'DINE_IN' && tableNumber ? (
            <button
              ...
              className="flex w-full items-center justify-center gap-1 rounded-lg border border-red-500/40 bg-red-950/40 py-1.5 text-[11px] font-semibold text-red-200 transition-colors hover:bg-red-600/30 active:bg-red-600/50"
            >
        ...
        <div
          className={`mx-3 mb-3 mt-3 flex shrink-0 items-center gap-3 px-3 ${gksClockBarClass()} ${kassaOrderTypeButtonTouchClass(
            kassaSxgaDenseTiles,
          )}`}
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-3 pt-1 touch-pan-y">
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              (parkedOnlySidebarView || numpadPanelVisible) ? (
                ...
                >
                  <p className={`mb-1 shrink-0 text-xs font-semibold uppercase tracking-wide ${ui.numpadMeta}`}>
              ...
              {parkedLinesByCategory.length > 0 ? (
                <div
                  ...
                  data-testid="kassa-parked-on-table"
                >
                  <p className={`mb-1 text-xs font-semibold uppercase tracking-wide ${ui.numpadMeta}`}>
                    {t('kassaApp.parkedOnTableSection')}
                  </p>
                  <div className="space-y-1">
                    {parkedLinesByCategory.map((item) => {
              ...
              <p
                className={`shrink-0 text-[10px] font-semibold uppercase tracking-wide ${ui.numpadMeta}`}
              >
              ...
              {cartLinesByCategory.map(item => {
                ...
                return (
                  <div key={item.cartKey} className={ui.cartRowBg}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-sm truncate ${ui.cartTitle}`}>{item.product.name}</p>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`w-6 text-center font-semibold text-base ${ui.numpadInput}`}>{item.quantity}</span>
                      <button
                        type="button"
                        onClick={() => updateQty(item.cartKey, item.quantity + 1)}
                        className={`touch-manipulation font-semibold flex items-center justify-center ${GKS_BTN_SHAPE} ${GKS_POS_BTN} ${
        ...
        >
          ...
          {orderType === 'DINE_IN' && tableNumber && cart.length > 0 && (
            <div
              className="flex w-full gap-2.5 touch-manipulation select-none"
          ...
          <div className="w-full touch-manipulation select-none border-t border-white/10 pt-3">
            <button
              ...
              className={`flex w-full items-center justify-center ${GKS_CHECKOUT_BTN} ${
                kassaSxgaDenseTiles
                  ? 'min-h-[4.25rem] py-4 text-2xl'
                  : 'min-h-[3.85rem] py-3.5 text-xl'
              }`}
            >
              {t('kassaApp.checkout')}
            </button>
          ...
          <div className={ui.modalConfirmBg}>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 font-semibold transition-colors ${GKS_ACCENT_BTN}`}
              >
      ...
      {effectiveAccess.reservaties && pendingReservCount > 0 && !showReservations && (
        ...
        >
          ...
          <div className="min-w-0">
            <p className="font-semibold text-xl md:text-2xl leading-tight">
        ...
        >
          ...
          <div
            className={`relative z-10 flex w-full max-w-3xl max-h-[min(88vh,780px)] min-h-0 flex-col overflow-hidden rounded-2xl border shadow-2xl ${ui.tablePickerBorder} ${
              kassaAppearanceDark ? GKS_MENU_PLATE_SHELL_BG_CLASS : 'bg-white'
            }`}
          >
            <div className={`flex shrink-0 items-center gap-3 border-b px-4 py-3 ${ui.tablePickerHeader}`}>
              <div className="min-w-0 flex-1 text-center">
                <p
                  ...
                  className={`text-xs font-semibold uppercase tracking-wider ${ui.tablePickerEmpty}`}
                >
                  {t('kassaApp.pickTableTitle')}
                </p>
                <p className={`mt-0.5 text-base font-semibold ${ui.flyMenuText}`}>
              ...
              <button
                ...
                className={`shrink-0 rounded-xl px-3 py-2 text-lg font-semibold leading-none transition-colors ${ui.categoryStripHover} ${ui.flyMenuText}`}
                aria-label={t('kassaApp.closeAria')}
              >
                ×
              </button>
                    ...
                    <div className="grid grid-cols-3 gap-3 p-4 sm:grid-cols-4 md:grid-cols-5">
                      {pickerTables.map((tbl) => (
                        <button
                          ...
                          className={`relative touch-manipulation border-2 py-4 font-semibold transition-colors active:brightness-95 ${GKS_BTN_SHAPE} ${
        ...
        >
          <div className={ui.printFallbackPanel}>
            ...
            <a
              ...
              className={`mt-4 flex w-full items-center justify-center px-4 py-3 text-center text-sm font-semibold ${GKS_ACCENT_BTN}`}
            >
        ...
        >
          <div className="max-w-lg rounded-2xl border border-red-200/80 bg-white px-8 py-10 text-center shadow-2xl">
            ...
            <h2
              ...
              className="text-xl font-semibold leading-snug text-red-700 sm:text-2xl"
            >
      ...
      {newOrderAlert && !demoViewOnly && (
        ...
        >
          <div className="text-white text-center px-8">
            ...
            <h1 className="text-4xl md:text-6xl font-semibold mb-4">{t('kassaApp.newOrderSplashTitle')}</h1>
            <div className="text-3xl md:text-5xl font-semibold mb-6">#{newOrderAlert.orderNumber}</div>
