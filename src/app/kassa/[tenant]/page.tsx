...
export default function KassaFullscreenPage({ params }: { params: { tenant: string } }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('kassa-fullscreen-orders')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <div className="flex h-[calc(100vh-64px)]">
                ...
                >
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-gray-400 text-xs">{getTimeSince(order.created_at)}</span>
                    </div>
        ...
        {/* Completed Orders - Thin Sidebar */}
        <div className="w-64 bg-gray-800 border-l border-gray-700 p-3 overflow-y-auto">
          <h2 className="text-sm font-bold mb-3 text-gray-400">AFGEROND ({completedOrders.length})</h2>
                  ...
                  <div className="space-y-2">
                    {selectedOrder.items?.map((item: any, i: number) => (
                      <div key={i} className="flex items-center justify-between py-2 border-b border-gray-600 last:border-0">
                        <div className="flex items-center gap-3">
                          <span className="w-8 h-8 bg-gray-600 rounded-lg flex items-center justify-center font-bold text-lg">
                            {item.quantity}
                          </span>
                          <span className="font-medium text-lg">{item.product_name || item.name}</span>
                        </div>
                        <span className="font-bold text-lg">€{(item.total_price || item.price * item.quantity)?.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
          ...
          >
            ...
            >
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-gray-700 rounded-xl border-none resize-none h-24 mb-6 text-white"
              />
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-red-500 hover:bg-red-600 rounded-2xl font-bold text-lg disabled:opacity-50"
                >
                  Afwijzen
                </button>
              </div>
