...
const HERO_BG = '/images/abonnement-gallery-pos-2.png'
...
export default function RetailLandingHero() {
  const [isMenuOpen, setIsMenuOpen] = useState(false)
  const { t } = useLanguage()

  const pillLinks: { href: string; label: string }[] = [
    { href: '/retail#sectoren', label: t('nav.sectors') },
    { href: '/retail#platform', label: t('nav.services') },
    { href: '/retail#prijzen', label: t('nav.pricing') },
  ]
      ...
      <header className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 pb-1">
        ...
        {isMenuOpen && (
          <div className="mt-4 rounded-2xl bg-black/50 backdrop-blur-md border border-white/20 p-4 space-y-1 sm:hidden">
            ...
            <MarketingStartAndDemoButtons demoHref="/retail#contact" onDark fullWidth className="mt-2 pt-2 border-t border-white/20" />
          </div>
        )}
      </header>
