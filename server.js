...
/** Pin 2 + pin 5 + DLE DC4 — zelfde bytes als open-drawer.ps1, maar via RAW zoals de bon. */
const DRAWER_KICK_ALL = Buffer.from([
  ESC, 0x70, 0x00, 0x32, 0x32,
  ESC, 0x70, 0x01, 0x32, 0x32,
  0x10, 0x14, 0x01, 0x00, 0x32,
  0x10, 0x14, 0x01, 0x01, 0x32,
])

function drawerKickPayload() {
  return Buffer.concat([Buffer.from([ESC, 0x40]), DRAWER_KICK_ALL])
}

function withDrawerKickInReceipt(buf) {
  if (buf.length >= 2 && buf[0] === ESC && buf[1] === 0x40) {
    return Buffer.concat([buf.subarray(0, 2), DRAWER_KICK_ALL, buf.subarray(2)])
  }
  return Buffer.concat([drawerKickPayload(), buf])
}
...
function resolveKassabonReviewUrl(body, opts) {
  const a = String(opts && opts.reviewUrl || '').trim()
  if (a) return a
  const b = String(body && body.reviewUrl || '').trim()
  if (b) return b
  return reviewUrlFromKassaUrl(opts && opts.kassaUrl)
}

function buildKassabonReviewQr(reviewUrl) {
  ...
  if (url) {
    try {
      const { buildTd80QrImage } = require('./td80-qr')
      parts.push(buildTd80QrImage(url))
    } catch (e) {
      console.warn('[review-qr]', e && e.message ? e.message : e)
    }
  }
  parts.push(Buffer.from('\n', 'latin1'))
  parts.push(enc('geef review'))
  parts.push(Buffer.from('\n', 'latin1'))
  return Buffer.concat(parts)
}
...
/** Zelfde QR als 123/TD80. Bonregels blijven gelijk; QR alleen onderaan kassabon. */
function appendKassabonReviewQr(buf, reviewUrl) {
  const qr = buildKassabonReviewQr(reviewUrl)
...
function buildEscPosPayload(body, opts) {
  const core = buildEscPosPayloadCore(body)
  if (body && body.receiptMode === 'keuken') return core
  const url = resolveKassabonReviewUrl(body, opts)
  if (!url) return core
  return appendKassabonReviewQr(core, url)
}

function buildEscPosPayloadCore(body) {
...
function executePrintRequest(body, cfg) {
  ...
  const printOpts = {
    kassaUrl: cfg && cfg.kassaUrl,
    reviewUrl: String(b.reviewUrl || (cfg && cfg.reviewUrl) || '').trim() || reviewUrlFromKassaUrl(cfg && cfg.kassaUrl),
  }
  ...
  const payloadToPrint =
    wantDrawer && receiptMode === 'kassa' ? withDrawerKickInReceipt(payloadMain) : payloadMain

  for (let i = 0; i < copies; i++) {
    const result = printRawWindows(targetPrinter, payloadToPrint)
    if (!result.ok) {
...
function createApp(
  ...
  app.post('/drawer', (_req, res) => {
    ...
    try {
      const r = printRawWindows(printerName, drawerKickPayload())
      if (!r.ok) {
        console.error('[drawer] RAW →', r.error)
